...
export const fetchAdministrativeLevelOnes = async (
  queryParams: {
    page?: number
    size?: number
  ...
  const {
    ...
    data: Ref<IPaginationResponse<IAdministrativeLevelOne> | null>
    error: Ref<unknown | null>
  } = await useFetch<IPaginationResponse<IAdministrativeLevelOne>>(
    `${baseUrl}/geo/administrative-levels-ones/?${params.toString()}`,
  )
    .get()
    .json()

  if (error.value) {
    console.error('Fetch administrative level ones failed:', error.value)
    return
  }

  console.log('Fetch administrative level ones successful:', data.value)
  return data.value
}
